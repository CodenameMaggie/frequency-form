 ...
 * DUPLICATE PREVENTION: Tracks all sent emails and prevents spam
 */
 ...
 * @param {boolean} options.skipDuplicateCheck - Skip duplicate prevention (use with caution!)
 * @param {string} options.relatedEntityType - Related entity type for tracking (contact, lead, order, etc.)
 * @param {string} options.relatedEntityId - Related entity ID
 * @param {string} options.sentBy - Who/what is sending (bot name, user email, or 'manual')
 * @param {string} options.tenantId - Tenant ID (defaults to F&F tenant)
...
async function sendEmail({
  ...
  try {
    ...
    if (!skipPreferenceCheck && emailType) {
      const canSend = await canReceiveEmail(normalizedEmail, emailType);
      if (!canSend) {
        console.log(`[Email Sender] ⏭️  Skipping email to ${normalizedEmail} - user unsubscribed from ${emailType}`);
        return {
          success: true,
          skipped: true,
    ...
    if (!skipDuplicateCheck) {
      // Generate deduplication key (unique identifier for this email)
      const dedupKey = `${emailType}:${normalizedEmail}${relatedEntityId ? ':' + relatedEntityId : ''}`;

      console.log(`[Duplicate Check] Checking if email can be sent...`);
      console.log(`[Duplicate Check] Type: ${emailType}, To: ${normalizedEmail}, Dedup Key: ${dedupKey}`);

      // Check if email can be sent (using database function)
      const { data: canSendResult, error: checkError } = await supabase
        .rpc('can_send_email', {
      ...
      if (checkError) {
        console.error('[Duplicate Check] Error checking duplicate:', checkError);
        // Continue anyway if duplicate check fails (fail open)
      } else if (canSendResult === false) {
        console.log(`[Duplicate Check] ⏭️  BLOCKED - Duplicate email detected`);
        console.log(`[Duplicate Check] Email type "${emailType}" to ${normalizedEmail} was recently sent`);
        ...
        await supabase.from('email_sent_log').insert({
          ...
          email_category: emailType.includes('confirmation') || emailType.includes('receipt') || emailType.includes('notification') ? 'transactional' : 'marketing',
          subject: subject,
          sent_from: from,
          sent_by: sentBy,
          dedup_key: dedupKey,
          ...
          delivery_status: 'blocked_duplicate',
          metadata: {
            reason: 'Duplicate prevention - email recently sent',
        ...
        return {
          ...
          skipped: true,
          blocked: true,
          reason: `Duplicate email prevented - "${emailType}" was recently sent to ${normalizedEmail}`,
          dedupKey: dedupKey
        };
      } else {
        console.log(`[Duplicate Check] ✅ Email approved - no recent duplicates found`);
      }
    }
    ...
    if (response.data && response.data.success) {
      ...
      // Log successful email send for duplicate prevention tracking
      if (!skipDuplicateCheck) {
        const dedupKey = `${emailType}:${normalizedEmail}${relatedEntityId ? ':' + relatedEntityId : ''}`;
        ...
        try {
          await supabase.from('email_sent_log').insert({
            ...
            recipient_email: normalizedEmail,
            email_type: emailType,
            email_category: emailType.includes('confirmation') || emailType.includes('receipt') || emailType.includes('notification') || emailType.includes('password') || emailType.includes('verification') ? 'transactional' : 'marketing',
            subject: subject,
            sent_from: from,
            sent_by: sentBy,
            dedup_key: dedupKey,
            ...
            provider: response.data.provider || 'forbes_command',
          ...
          console.log(`[Email Log] ✅ Email logged for duplicate prevention`);
        } catch (logError) {
          // Don't fail the email send if logging fails
          console.error('[Email Log] ⚠️  Failed to log email:', logError.message);
        }
      }
